...
MSRC_CVRF_PANDAS_PRODUCTS = Path(
    DATA_DIR, "msrc-pandas-cvrf-products-map.json.gz")
...
def get_kb_and_version(rem):
    """
    Builds datable of KB and version
    """
...
def get_versions(rems):
    """
    MSRC List of Software Versions related to CVE
    """
...
def get_types(threats):
    """
    MSRC List of Threat Types related to CVE
    """
    ...
    for ack in acks:
        if ack.get('Name'):
            for name in ack['Name']:
                if name.get('Value'):
                    researchers.append(name['Value'].replace(
                        '\n', ''))  # remove new lines
    ...
    for item in items:
        if item.get('ProductID'):
            for id in item['ProductID']:
                product_names.add(prod_map[id]['product'])
...
def get_product_types(items, prod_map):
    ...
    return sorted(list(set(product_types)))
...
def create_msrc_cvrf_pandas():
    ...
    FIELDS = ["Initial Release", "Tag", "Title", "Impact",
              "CVSS", "KBs", "Products", "Versions", "Acks"]
    ...
    if should_update(MSRC_CVRF_PANDAS_FULL, 1):
        ...
        for cvrf_id in msrc_merged_json:
            ...
            df_vulns = pd.json_normalize(
                msrc_merged_json[cvrf_id]['Vulnerability'])
            ...
            for item in prods['Items']:
                ...
                for prod in item['Items']:
                    product_cat[prod['ProductID']] = {
                        'product': prod['Value'], 'category': category}
            ...
            df_vulns['KB URLs'] = df_vulns["Remediations"].apply(get_kb_urls)
            ...
            df_vulns['Initial Release'] = datetime.strftime(datetime.fromisoformat(
                df_update['DocumentTracking.InitialReleaseDate'].values[0].replace('Z', '')), '%Y-%m-%d')
            df_vulns['Current Release'] = datetime.strftime(datetime.fromisoformat(
                df_update['DocumentTracking.CurrentReleaseDate'].values[0].replace('Z', '')), '%Y-%m-%d')
            df_vulns['Acks'] = df_vulns['Acknowledgments'].apply(get_acks)
            df_vulns['CVSS'] = df_vulns['CVSSScoreSets'].apply(get_cvss_base)
            df_vulns['Products'] = df_vulns['ProductStatuses'].apply(
                get_products, args=(product_cat,))
            df_vulns['Product Types'] = df_vulns['ProductStatuses'].apply(
                get_product_types, args=(product_cat,))
        ...
        all_products_df = all_products_df.reset_index(names='id')
...
def create_msrc_tags_titles():
    ...
    tags_title_df = tags_df.reset_index().groupby(
        ['Tag', 'Title']).aggregate(set)
    print(tags_title_df.head())
    copy_tags_df = tags_df.copy()
    tags_df['MSRC Count'] = tags_df['Tag'].apply(
        lambda x: len(copy_tags_df[copy_tags_df['Tag'] == x]))
...
def create_kb_ver():
    ...
    if should_update(ALL_KBS_VERSION_MAP, 1):
        all_cvrf_df = pd.read_json(MSRC_CVRF_PANDAS_FULL)
        rems = pd.Series(all_cvrf_df['Remediations'].explode(
            'Remediations'))  # .dropna().reset_index(drop=True)
    ...
    kb_vers_df['Build'] = kb_vers_df['Version'].apply(
        lambda x: x.split('.')[-2] if (len(x.split('.')) == 4) else None)
